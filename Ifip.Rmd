...
snapshots <- dataset %>% group_by(profileAlign) %>% summarise(snapshots = n_distinct(timelineId))
...
test <- teste %>% group_by(yday,profileAlign) %>% summarize(snapshots=n_distinct(timelineId)) 
test <- test %>% mutate(impressionDay=ymd_hms("2018-01-01 12:00:00")+days(yday))  
test %>%   ggplot(aes(x = impressionDay,y=snapshots,fill=profileAlign)) + geom_bar(stat="identity")+ theme(axis.text.x = element_text(angle = 0, hjust = 1),legend.position="bottom",axis.title.x=element_blank(),text = element_text(size=12))
...
ttlClassUser <- results %>% group_by(profileAlign,Likes,top_position) %>% 
...
(ggplot(filter(results), aes(x=top_position,y=prop_occupancy,fill=Likes)) +   geom_bar(stat="identity",position="fill")+ facet_wrap(~profileAlign)+theme_bw()
 +labs(x="News Feed size", y="Normalized occupancy")+ theme(legend.position="bottom",axis.text = element_text( size=12, face="bold"),axis.title=element_text(size=12,face="bold"))+guides(fill=guide_legend(title="Likes"))
)
...
(ggplot(filter(results), aes(x=top_position,y=visibility,fill=Likes)) +   geom_bar(stat="identity",position="fill")+ facet_wrap(~profileAlign)+theme_bw()
...
(ggplot(filter(results), aes(x=top_position,y=visibility,fill=Likes)) +   geom_bar(stat="identity")+ facet_wrap(~profileAlign)+theme_bw()
 +labs(x="News Feed size", y="Visiblity")+ theme(legend.position="bottom",axis.text = element_text( size=12, face="bold"),axis.title=element_text(size=12,face="bold"))+guides(fill=guide_legend(title="Likes"))
)

ggsave("visibility_versus_top_position.eps")


(ggplot(filter(results), aes(x=top_position,y=visibility,fill=publisherOrientation)) +   geom_bar(stat="identity")+ facet_wrap(~profileAlign)+theme_bw()
 +labs(x="News Feed size", y="Visiblity")+ theme(legend.position="bottom",axis.text = element_text( size=12, face="bold"),axis.title=element_text(size=12,face="bold"))+guides(fill=guide_legend(title="Likes"))
)
...
(ggplot(filter(results,top_position=='1' & views >5), aes(x=reorder(publisherName,-created_posts),y=views,fill=Likes))  + facet_wrap(~profileAlign,ncol=2)
...
(ggplot(filter(results,top_position=='10' & occupancy/top > 0.015), aes(x=reorder(publisherName,-created_posts),y=views,fill=Likes))  + facet_wrap(~profileAlign)
...
(ggplot(filter(results,top_position=='20' & occupancy/top > 0.015), aes(x=reorder(publisherName,-created_posts),y=views,fill=Likes))  + facet_wrap(~profileAlign)
...
(ggplot(filter(results,top_position=='1' & views >5), aes(x=reorder(publisherName,-created_posts),y=visibility,fill=Likes))  + facet_wrap(~profileAlign)
    +geom_bar(stat="identity") +theme_bw()+ theme(axis.text.x = element_text(angle = 90, hjust = 1,vjust=0.5),legend.position="bottom",axis.title.x=element_blank(),text = element_text(size=8,face="bold"))
  +labs( y="Visibility")
  +guides(fill=guide_legend(title="Likes"))
...
(ggplot(filter(results,top_position=='10' & occupancy/top > 0.015), aes(x=reorder(publisherName,-created_posts),y=visibility,fill=Likes))  + facet_wrap(~profileAlign)
...
(ggplot(filter(results,top_position=='30' & occupancy/top > 0.015), aes(x=reorder(publisherName,-created_posts),y=visibility,fill=Likes))  + facet_wrap(~profileAlign)
    +geom_bar(stat="identity") +theme_bw()+ theme(axis.text.x = element_text(angle = 90, hjust = 1,vjust=0.5),legend.position="bottom",axis.title.x=element_blank(),text = element_text(size=9,face="bold"))
  +labs( y="Visibility")
  +guides(fill=guide_legend(title="Likes"))
  +geom_point(aes(x=publisherName,y=created_posts/9000), size = 1) 
 +scale_y_continuous(sec.axis = sec_axis(~.*9000, name = "Created Posts"))
...
(ggplot(filter(Fairness,top==30 & fairnessType=="ProportionalFairness" & profileAlign!="undecided"), aes(x=reorder(publisherName,-created_posts),y=fairnessOccupancy,colour=weight,group=weight))+geom_line()
 +theme_bw()+ theme(axis.text.x = element_text(angle = 90, hjust = 1,vjust=0.5),legend.position="bottom",axis.title.x=element_blank(),text = element_text(size=8,face="bold"))+labs( y="Occupancy")+guides(colour=guide_legend(title="Liked class weight"))
 +facet_wrap(~profileAlign,ncol=2)
)
ggsave("propFairness_por_peso_no_top30.eps")


(ggplot(filter(Fairness,top==30, fairnessType=="ProportionalFairness", weight==1), aes(x=reorder(publisherName,-created_posts),y=fairnessOccupancy,fill=Likes))  + facet_wrap(~profileAlign)
...
(ggplot(filter(Fairness,top==1 & weight==2  ), aes(x=reorder(publisherName,-created_posts),y=fairnessOccupancy,colour=fairnessType ,shape=fairnessType))
 +geom_point()
  +theme_bw()+ theme(axis.text.x = element_text(angle = 90, hjust = 1,vjust=0.5),legend.position="bottom",axis.title.x=element_blank(),text = element_text(size=8,face="bold"))+labs( y="Occupancy",colour  = "Fairness Type", shape = "Fairness Type")
...
(ggplot(filter(Fairness,top==1 & weight==2 & fairnessType!="Facebook"), aes(x=reorder(publisherName,-created_posts),y=fairnessBiasFB,colour=fairnessType,shape=fairnessType))+geom_point()
 +theme_bw()+ theme(axis.text.x = element_text(angle = 90, hjust = 1,vjust=0.5),legend.position="bottom",axis.title.x=element_blank(),text = element_text(size=8,face="bold"))+labs( y="Bias",colour  = "Fairness Type", shape = "Fairness Type")
 +facet_wrap(~profileAlign)
)
...
(ggplot(filter(Fairness,top==1 & weight==1 & fairnessType=="ProportionalFairness"), aes(x=reorder(publisherName,-created_posts),y=fairnessBias,fill=Likes))+geom_bar(stat="identity")  
 +theme_bw() + theme(axis.text.x = element_text(angle = 90, hjust = 1,vjust=0.5),legend.position=c(0.89,0.39),axis.title.x=element_blank(),text = element_text(size=9,face="bold"), legend.direction = "horizontal",legend.text=element_text(size=9))
...
(ggplot(filter(Fairness,top==30 & weight==1 & fairnessType=="ProportionalFairness"), aes(x=reorder(publisherName,-created_posts),y=fairnessBias/top,fill=Likes))+geom_bar(stat="identity")  
 +theme_bw() + theme(axis.text.x = element_blank(),legend.position="none",axis.title.x=element_blank(),text = element_text(size=8,face="bold"))
...
```{r}
tmp <-  Fairness %>% filter(top==1 & weight==1 ) %>% group_by(profileAlign,Likes,fairnessType) %>% summarise(fairnessBias=sum(fairnessBias),fairnessBiasFB=sum(fairnessBiasFB)) 
...
(ggplot(tmp,aes(x=Likes,y=fairnessBias,colour=fairnessType))+geom_point()
 ...
 +facet_wrap(~profileAlign)+coord_flip()
)
...
filter(Fairness,top==1, weight==10) %>% group_by(profileAlign,fairnessType) %>% summarize(bias=sum(fairnessBias),occupancy=sum(fairnessOccupancy))

filter(Fairness,top==1,weight==10) %>% group_by(profileAlign,fairnessType) %>% summarize(occupancy=sum(unfiltered_probability))

Fairness <- Fairness %>% mutate(TTL=fairnessOccupancy/creationRate) 


(ggplot(filter(Fairness,fairnessType=="ProportionalFairness", top==30 , weight==1), aes(x=reorder(publisherName,-created_posts),y=TTL,fill=Likes))  + facet_wrap(~profileAlign,ncol=2)
  +geom_bar(stat="identity")  
