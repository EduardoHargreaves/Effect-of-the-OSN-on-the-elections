...
(ggplot(filter(results,top_position=='1' & views >5), aes(x=reorder(publisherName,-created_posts),y=views,fill=Likes))  + facet_wrap(~profileAlign)
    +geom_bar(stat="identity") +theme_bw()+ theme(axis.text.x = element_text(angle = 90,vjust=0.5, hjust=1),legend.position=c(.945,.88),axis.title.x=element_blank(),text = element_text(size=8.5,face="bold"),legend.text=element_text(size=9))
  +labs( y="Impressions")
...
(ggplot(filter(Fairness,fairnessType=="ProportionalFairness", top==30 , weight==1), aes(x=reorder(publisherName,-created_posts),y=TTL,fill=Likes))  + facet_wrap(~profileAlign)
  +geom_bar(stat="identity")  
